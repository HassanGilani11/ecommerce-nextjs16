...
export default function EditTagForm({ tag }: EditTagFormProps) {
    ...
    const handleSave = () => {
        ...
        startTransition(async () => {
            const result = await updateTag(tag.id, null, formData)
            if (result?.error) {
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tighter">Edit Tag</h1>
                        <p className="text-zinc-500 text-sm">Update tag details.</p>
                    </div>
                </div>
                ...
                <div className="space-y-8">
                    <Card className="border-none shadow-sm rounded-[2.5rem] bg-zinc-900 text-white overflow-hidden p-8">
                        ...
                        <p className="text-zinc-400 text-sm leading-relaxed">
                            Editing this tag will update it across all products that currently use it. The slug change may affect SEO URLs.
                        </p>
                    </Card>
                </div>
            </div>

...
export default function EditCategoryForm({ category }: EditCategoryFormProps) {
    ...
    const [parent, setParent] = React.useState(category.parent_id || "none")
    const [imageUrl, setImageUrl] = React.useState(category.image_url || "")
    ...
    React.useEffect(() => {
        const fetchParents = async () => {
            ...
            const { data } = await supabase
                .from('categories')
                .select('id, name')
                .neq('id', category.id) // Exclude self
                .order('name')
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tighter">Edit Category</h1>
                        <p className="text-zinc-500 text-sm">Update category details and organization.</p>
                    </div>
                </div>

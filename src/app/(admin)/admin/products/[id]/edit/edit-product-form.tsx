...
const TAG_OPTIONS = ["New Arrival", "Bestseller", "Limited Edition", "Eco-Friendly", "Premium", "Organic"]
...
export default function EditProductForm({ product }: EditProductFormProps) {
    ...
    const [category, setCategory] = React.useState(product.category?.slug || "")
    const [brand, setBrand] = React.useState(product.brand?.slug || "none")
    const [selectedTags, setSelectedTags] = React.useState<string[]>(
        product.tags.map(t => t.tag.name)
    )
    const [imageUrl, setImageUrl] = React.useState(product.featured_image || "")
    const [galleryImages, setGalleryImages] = React.useState<string[]>(
        Array.isArray(product.gallery) ? product.gallery.map(String) : []
    )
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-zinc-500 text-sm">Update product details and inventory.</p>
                    </div>
                </div>
